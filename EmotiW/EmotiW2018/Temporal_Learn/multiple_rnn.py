...
print("Load data...")
X_TRAIN_PATH = TRAIN_PATH + 'x_dense_fc6_frames_overlap.npz'
Y_TRAIN_PATH = TRAIN_PATH + 'y_dense_fc6_frames_overlap.npz'
...
def MultipleRNN(x, weight, bias):
    stack_cell = []
    if CELL == 'mult_lstm':
        for _ in range(num_layer):
            stack_cell.append(
                rnn.DropoutWrapper(rnn.BasicLSTMCell(num_hidden, forget_bias=1.0), output_keep_prob=dropout))
            mult_cell = rnn.MultiRNNCell(stack_cell, state_is_tuple=True)
    elif CELL == 'mult_gru':
        for _ in range(num_layer):
            stack_cell.append(
                rnn.DropoutWrapper(rnn.GRUCell(num_hidden), output_keep_prob=dropout))
            mult_cell = rnn.MultiRNNCell(stack_cell, state_is_tuple=True)

    outputs, states = tf.nn.dynamic_rnn(mult_cell, x, dtype=tf.float32)
    outputs = tf.transpose(outputs, [1, 0, 2])
    last = tf.gather(outputs, int(outputs.get_shape()[0]) - 1)

    return tf.matmul(last, weight) + bias

weight = tf.Variable(tf.truncated_normal([num_hidden, num_classes], stddev=0.1), name='W')
bias = tf.Variable(tf.truncated_normal([num_classes]), name='b')

logits = MultipleRNN(X, weight, bias)
...
#        learning_rate = 1e-3
        if step <= 15000:
            learning_rate = 1e-3
        elif step > 15000 and step < 25000:
            learning_rate = 1e-4
        elif step >= 25000:
            learning_rate = 1e-5
#        elif step > 20000:
